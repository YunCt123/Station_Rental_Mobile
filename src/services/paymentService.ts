...
import { CreatePayOSPaymentRequest, CreatePayOSPaymentResponse, PaymentsResponse, PaymentResponse } from "../types/payment";
...
export const paymentService = {
  ...
  async createDeposit(
    ...
    try {
      console.log('[paymentService.createDeposit] Calling API:', bookingId, amount);
      ...
      console.error('[paymentService.createDeposit] Error response:', error.response?.data);
      throw error;
    }
  },
  ...
  async createVNPAYDeposit(
    ...
    try {
      console.log('[paymentService.createVNPAYDeposit] Calling API:', bookingId, amount);
      // Use same endpoint as createDeposit but indicate provider
      const res = await api.post(
        `${PAYMENT_ENDPOINTS.CREATE_DEPOSIT(bookingId)}`,
        { amount, provider: 'VNPAY' }
      );
      console.log('[paymentService.createVNPAYDeposit] Response:', res.data);
      
      const data = res.data?.data || res.data;
      return {
        checkoutUrl: data?.checkoutUrl || data?.vnpay_url,
  ...
  async createPayOSPayment(payload: CreatePayOSPaymentRequest): Promise<CreatePayOSPaymentResponse> {
    try {
  ...
  async handlePayOSCallback(params: Record<string, any>): Promise<PaymentResponse> {
  ...
  async handlePayOSClientCallback(params: {
    ...
    try {
      console.log('[paymentService.handlePayOSClientCallback] Calling API:', params);
      const res = await api.post(PAYMENT_ENDPOINTS.PAYOS_CLIENT_CALLBACK, params);
      console.log('[paymentService.handlePayOSClientCallback] Response:', res.data);
  ...
  async handleVnpayCallback(params: Record<string, any>): Promise<PaymentResponse> {
    try {
